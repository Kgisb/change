...
with st.sidebar:
    ...
    # Friendly descriptions & chip preview of views in the selected Section
    SECTION_DESCRIPTIONS = {
        "Performance": "Status & run-rate views for revenue and ops health.",
        "Funnel & Movement": "Lead flow, stuck deals, velocity, and conversion stages.",
        "Insights & Forecast": "Predictibility, projections, Pareto, trends, and visual analyses.",
        "Marketing": "Referrals, HubSpot scoring, and lead performance needs.",
    }
    desc = SECTION_DESCRIPTIONS.get(master, '')
    st.markdown(f"<div style='margin:6px 0 2px; font-size:12px; opacity:.85'>Section overview</div>", unsafe_allow_html=True)
    st.markdown(f"<div style='border:1px solid #e7e8ea;border-radius:12px;padding:10px;background:#fff'>{desc}</div>", unsafe_allow_html=True)
    
    # Chip-style preview of sub-views (non-interactive; radio below controls selection)
    chips = ''.join([f"<span style='display:inline-block;margin:6px 6px 0 0;padding:6px 10px;border:1px solid #e7e8ea;border-radius:999px;font-size:12px;background:{'#111827' if v==('' if 'nav_sub' not in st.session_state else st.session_state.get('nav_sub')) else '#f8fafc'};color:{'#fff' if v==st.session_state.get('nav_sub') else '#0f172a'}'>{v}</span>" for v in MASTER_SECTIONS.get(master, [])])
    st.markdown(f"<div style='margin-top:6px'>{chips}</div>", unsafe_allow_html=True)
    st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
    # --- Clickable 'pill' tabs for sub-views (no redundancy) ---
    ...
    cur_sub = st.session_state['nav_sub']
    st.markdown("<div style='margin:6px 0 2px; font-size:12px; opacity:.85'>Views</div>", unsafe_allow_html=True)
    pill_area = st.container()
    with pill_area:
        for v in sub_views:
            is_active = (v == cur_sub)
            btn = st.button(v, key=f'navpill_{v}', use_container_width=True)
            if btn:
