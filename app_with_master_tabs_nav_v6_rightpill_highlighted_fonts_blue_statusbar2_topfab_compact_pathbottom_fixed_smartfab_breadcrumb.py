...
def _render_status_bar(excluded_count: int, excluded_col: str, rows_in_scope: int, track_val: str):
    ...
    <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; margin:6px 0 10px;">
      <span style="display:inline-flex; align-items:center; gap:6px;
                   ...
                   background:#F9FAFB; font-size:12.5px;">
        <strong style="font-weight:600;">Excluded</strong>
        <span>“1.2 Invalid deal(s)”</span>
        <span style="opacity:.8">·</span>
        <span>{excluded_count:,} rows</span>
        <span style="opacity:.6">({excluded_col})</span>
      </span>
      <span style="display:inline-flex; align-items:center; gap:6px;
                   padding:6px 10px; border-radius:999px; border:1px solid #1E40AF;
                   background:#1D4ED8; color:#fff; font-size:12.5px;">
        <strong style="font-weight:600;">In scope</strong>
        <span>{rows_in_scope:,}</span>
      </span>
      <span style="display:inline-flex; align-items:center; gap:6px;
                   padding:6px 10px; border-radius:999px; border:1px solid #E5E7EB;
                   background:#F9FAFB; font-size:12.5px;">
        <strong style="font-weight:600;">Track</strong>
        <span>{track_val}</span>
      </span>
    </div>
    """
...
try:
    ...
    st.markdown(
        f"""<div style='margin:4px 0 8px; font-size:12.5px; color:#334155;'>
        <span style='opacity:.9'>{_master or ''}</span>
        <span style='opacity:.5'> &nbsp;›&nbsp; </span>
        <strong style='font-weight:600'>{_view or ''}</strong>
...
for i, v in enumerate(sub_views):
    with cols[i % len(cols)]:
        ...
        if is_active:
            st.markdown(
                f"""<div style='display:block; width:100%; text-align:center;
                padding:8px 12px; border-radius:999px; border:1px solid #1E40AF;
                background:#1D4ED8; color:#fff; font-weight:600;'>
                {v}</div>""", unsafe_allow_html=True
            )
        else:
