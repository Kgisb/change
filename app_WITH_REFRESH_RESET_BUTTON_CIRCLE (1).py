...
with st.container():
    ...
    with _cols[-1]:
        ...
        if st.button("↻", key="refresh_all_btn", use_container_width=True):
...
# ---- Refresh button styles (scoped, circular) ----
try:
    import streamlit as st
    st.markdown("""
<style>
...
#refresh-ctl .stButton > button {
    ...
    font-weight: 800; font-size: 18px;  /* thicker icon */
    box-shadow: 0 10px 22px rgba(2,132,199,0.28);
    cursor: pointer;
    transition: transform .08s ease, box-shadow .12s ease, opacity .12s ease;
}
#refresh-ctl .stButton > button:hover { transform: translateY(-1px); box-shadow: 0 14px 30px rgba(2,132,199,0.34); }
#refresh-ctl .stButton > button:active { transform: translateY(0); box-shadow: 0 8px 16px rgba(2,132,199,0.28); }
</style>
""", unsafe_allow_html=True)
except Exception:
    pass
