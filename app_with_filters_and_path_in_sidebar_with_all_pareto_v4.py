...
if view == "MIS":
    ...
    if source_col:
        ...
        picked_sources = st.multiselect("Include Deal Sources (Pareto)", options=["All"] + ["All"] + sources_for_pick, default=sources_for_pick, key="eighty_picked_src")
    else:
        # Normalize Pareto selection: 'All' or empty => all sources
        try:
            _df_src = df80 if 'df80' in globals() else df
            all_sources__ = sorted([str(x) for x in _df_src[source_col].dropna().unique()]) if source_col else []
            if (not picked_sources) or ('All' in picked_sources):
                picked_sources = all_sources__
        except Exception:
            pass
        picked_sources = (sorted([str(x) for x in df[source_col].dropna().unique()]) if ('All' in picked_sources) else picked_sources)
        picked_sources = None
