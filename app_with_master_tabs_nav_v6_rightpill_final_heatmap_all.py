...
# =========================
    ...
    def _bubble_explorer():
        ...
        with f2:
            cty_pick = st.multiselect("Filter Country", options=["All"] + ["All"] + cty_all, default=["All"], help="Choose one or more. 'All' selects everything.")
        with f3:
            # Normalize: 'All' or empty => all countries for Heatmap
            try:
                _df_heat = df if 'df' in globals() else (df_f if 'df_f' in globals() else None)
                if _df_heat is not None:
                    _country_col = country_col if 'country_col' in globals() else 'Country'
                    if hasattr(_df_heat, 'columns') and (_country_col in getattr(_df_heat, 'columns', [])):
                        _all_countries = sorted([str(x) for x in _df_heat[_country_col].dropna().unique()])
                    else:
                        _all_countries = []
                    if (not cty_pick) or ('All' in cty_pick):
                        cty_pick = _all_countries
            except Exception:
                pass
