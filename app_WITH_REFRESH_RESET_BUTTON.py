...
with _cols[-1]:
    if st.button("↺  Refresh", key="refresh_all_btn", use_container_width=True):
        _reset_all_filters_and_cache(preserve_nav=True)
        st.rerun()
# Decorative pill (static) to keep layout balanced above button
st.markdown('<div class="refresh-wrap"><span class="refresh-btn"><span class="refresh-icon">↺</span>Refresh</span></div>', unsafe_allow_html=True)
...
# ---- Refresh pill button (UI only) ----
try:
    import streamlit as st
    st.markdown("""
<style>
.refresh-wrap { display:flex; justify-content:flex-end; align-items:center; margin-top:-6px; margin-bottom:6px; }
.refresh-btn {
    display:inline-flex; align-items:center; gap:8px;
    padding:8px 12px; border-radius:999px;
    background:#0ea5e9;                /* sky-500 */
    color:#fff; border:1px solid #0284c7; /* sky-600 */
    font-weight:700; font-size:12.5px;
    box-shadow: 0 8px 22px rgba(2,132,199,0.25);
    cursor:pointer; user-select:none;
    transition: transform .08s ease, box-shadow .12s ease, opacity .12s ease;
}
.refresh-btn:hover { transform: translateY(-1px); box-shadow: 0 12px 28px rgba(2,132,199,0.32); }
.refresh-btn:active { transform: translateY(0); box-shadow: 0 6px 16px rgba(2,132,199,0.28); }
.refresh-icon { display:inline-block; font-weight:900; }
@keyframes spin-slow { from{ transform:rotate(0deg);} to{ transform:rotate(360deg);} }
.refresh-anim { animation: spin-slow 1.2s linear infinite; }
</style>
""", unsafe_allow_html=True)
except Exception:
    pass
